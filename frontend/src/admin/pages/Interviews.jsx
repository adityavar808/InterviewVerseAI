...
const Interviews = () => {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await adminService.deleteInterview(
        item._id,
      );
      toast.success(
        "Interview template deleted",
      );
      loadInterviews();
    } catch (requestError) {
      toast.error(
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-6 rounded-[32px] border border-white/10 bg-[linear-gradient(135deg,rgba(8,145,178,0.14),rgba(15,23,42,0.82),rgba(245,158,11,0.08))] p-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">
            Build reusable interview tracks, control publication status, and keep question counts and durations aligned with the product.
          </p>
        </div>
                ...
                <tbody>
                  {interviews.length ? (
                    ...
                    <tr>
                      ...
                      >
                        No interview templates match the current filters.
                      </td>
                    </tr>
                  )}
                </tbody>
